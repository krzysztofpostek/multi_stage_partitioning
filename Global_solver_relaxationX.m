...
cvx_begin
    variable x(T-1,Number_of_subproblems) 
    ...
    dual variable v{T*Number_of_subproblems}
    dual variable eta{T*Number_of_subproblems}
    ...
    for iterate_problem=1:Number_of_subproblems
        ...
        % Objective function
        v{T*(iterate_problem-1)+1} : (T-1)*c_b*I_1 + Objective_x'*x(:,iterate_problem)  + Objective_y_holding_cost'*y(:,iterate_problem) + sum(Objective_y_ordering_cost(:,iterate_problem))  + sum([p; Extra_intercepts(:,iterate_problem)].*lambda(:,T*(iterate_problem-1)+1)) <= cost;
        eta{T*(iterate_problem-1)+1} : [P ; Extra_hyperplanes(:,:,iterate_problem)]'*lambda(:,T*(iterate_problem-1)+1) >= Objective_xi;
        ...
        for t=1:T-1
            v{T*(iterate_problem-1)+1+t} : -sum(x(1:t,iterate_problem))-I_1-sum(y(1:t,iterate_problem)*q) + sum(lambda(:,T*(iterate_problem-1)+1+t).*[p ;Extra_intercepts(:,iterate_problem)]) <=0;
            eta{T*(iterate_problem-1)+1+t} : [P ; Extra_hyperplanes(:,:,iterate_problem)]'*lambda(:,T*(iterate_problem-1)+1+t) >= [ones(t+1,1) ; zeros(T-t-1,1)];
        end
    ...
    x >= zeros(T-1,Number_of_subproblems);
