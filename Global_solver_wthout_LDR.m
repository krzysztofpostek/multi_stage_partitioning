cvx_begin
    ...
    variable y((T-1)*N,Number_of_subproblems) binary
    ...
    for iterate_problem=1:Number_of_subproblems
        ...
        % Objective function
        (T-1)*c_b*I_1 + Objective_x'*x(:,iterate_problem) + Objective_y'*y(:,iterate_problem) + sum([p; Extra_intercepts(:,iterate_problem)].*lambda(:,T*(iterate_problem-1)+1))<= cost;
        [P ; Extra_hyperplanes(:,:,iterate_problem)]'*lambda(:,T*(iterate_problem-1)+1) >= Objective_xi;
        
        % Nonnegativity of the inventory state
        for t=1:T-1
            -sum(x(1:t,iterate_problem))-I_1-sum(y(1:t*N,iterate_problem).*q(1:t*N)) + sum(lambda(:,T*(iterate_problem-1)+1+t).*[p ;Extra_intercepts(:,iterate_problem)]) <=0;
    ...
    y >= zeros((T-1)*N,Number_of_subproblems);
